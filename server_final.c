		...
		if(!strcmp(msg, sig_file))
		{
			...
			for(j=0; j<clnt_cnt; j++) {
				...
				if(!strcmp(tmpName, clnt_names[j]) ) {
					noCli = 0;
					fileGo = j; // Save the socket number to send
					break;
				}
				else if(j == clnt_cnt - 1) {
					noCli = 1; // mark if there is no client
					break;
				}
			}
			...
			for(j=0; j<clnt_cnt; j++) {
				if(!strcmp(tmpName, clnt_names[j]) ) {
					noCli = 0;
					mGo = j; // save the socket number to send
					break;
				}
				else if(j == clnt_cnt - 1) {
					noCli = 1; // when there is no client
					break;
				}
			}
			pthread_mutex_unlock(&mutx);
			...
			} // there's no client to whisper
			else {
				write(clnt_socks[j], msg, BUF_SIZE);
			} // there is client to whisper
